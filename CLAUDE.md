...
## Zoom feature-parity pass (after the workspace sections)
- Backend: `migrations.py` (ALTER TABLE for later columns); `models/core.py` adds Recurrence (meetings.recurrence/recurrence_end), MeetingSettings.allow_unmute/allow_rename, ChatMessage.recipient_id, and Poll/PollOption/PollVote. Services: `polls.py`, `recurrence.py` (occurrences, next_occurrence_after); meetings.py adds PMI instant meetings (`use_pmi`), recurring roll-forward (upcoming, calendar expansion, `_close_session`), and the lock check (router passes `manager.is_locked`)
- Realtime rewrite: `manager.py` LiveMeeting (sockets, location = breakout room per participant, screen_sharers per room, raised_hands, host_ids, cohost_ids, waiting, admitted, locked, spotlight_id, recording_ids, breakout_rooms); `session.py` Context(room_id, is_moderator), send_room_state, enter_room, move_to_room, broadcasts; `handlers.py` uses `@on(type, role=None|"moderator"|"host")`. New messages: rename, caption, stop_video, ask_start_video, lower_all_hands, lower_hand, spotlight, set_cohost, security, recording, poll_create/vote/end/share, breakout_open/assign/join/close; chat has `to_id`. Server → client adds force_video_off, video_request, spotlight, cohost, security, recording, caption, poll, breakout_state; room_state is sent again when changing rooms
